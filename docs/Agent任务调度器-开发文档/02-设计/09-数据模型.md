...
| `value_json` | TEXT | NOT NULL | `gates`：`{dispatch, review, landing}` 各 `auto` / `manual`；`pipeline`：`{bughunt: 0|1, wrapupMode: 'auto'|'manual'}` |
...
缺 `pipeline` 行时同法返回 `{bughunt: 0, wrapupMode: 'auto'}`；PATCH 两字段整体写入，写成功发 `settings.pipeline_changed`（E-318，决策 93）。
