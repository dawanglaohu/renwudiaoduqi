...
| `assignment_draft_json` | TEXT | NULL | 逐任务指派草稿 `{agentId, model, effort, draftedAt}`（10 节 `TaskAssignmentDraft`），`POST /batches/:id/assignments` 整批覆写、派发前可反复改；tick 派实施运行时 `resolveAgentForTask()` 先读它、无草稿才回落第一个可派 agent；派发快照写 `assignment_json` 时逐字抄它（M8-T9 落地后走同一入口），派出后草稿保留作历史、不清空（决策 136，E-108、E-31，M8-T11）；普通 `ADD COLUMN` |
...
| `session_no` | INTEGER | NULL | 会话序号：INSERT 时 = 该 `agent_id` 当时占并发额度的运行数（`countsTowardAgentConcurrency` 为真的行）+ 1，终态后保留不回收；同一 agent 同时跑两条会话就是 #1 / #2，随 `RunDto.sessionNo` 下发，前端不自算（决策 136，E-31，M8-T11）；普通 `ADD COLUMN`，旧行 NULL 读作 `null` 显示「—」 |
...
9. **接线审计 2026-09-19 的两个新列**（`tasks.assignment_draft_json`、`runs.session_no`）同样走普通 `ADD COLUMN`，同一个迁移文件归 M8-T11；默认 NULL、旧库直开不回填；M8-T9 落地时 `assignment_json` 的取值入口改为先读草稿列，不新增第二份草稿存储
