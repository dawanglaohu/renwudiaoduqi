...
**每个模块的产出最终都要挂到某个入口上，否则单测全绿也是死代码。** 下表是本仓库全部「注册点」——
路由表、容器装配件、路由装配件、构建管线、共享类型、事件 kind、迁移目录、后台 job 数组、壳桥接。
产出路由 / service / 页面 / job / 事件 / 迁移的任务，必须把对应注册点写进自己的有效范围
（`task-contracts.json` 的 `wiring` 类别或 `supportPaths`），验收标准必须写「从入口可达」的可判定形式；
本表同步镜像进 `_run/presentation.json` 的 `handoff.wiring`，任务声明的类别按此并入有效范围与契约哈希。
接线审计（2026-09-19，见 21 节 R-25）确认此前五条链路无人认领，已由 M9-T24～M9-T26、M8-T10、M8-T11、M7-T9、
M2-T7、M2-T8、M10-T6、M1-T11 认领。

| 类别 | 注册点 | 谁来核 |
|---|---|---|
| `backendRoutes` | `packages/daemon/src/http/plugins/50-routes.ts`（唯一路由注册插件，`ROUTES` 之外不得再挂）、`packages/shared/src/api/routes.ts`（10 节端点表的投影） | `test/arch/routes.test.ts`：实际注册集合 === `ROUTES`，且每条 handler 都是真实实现 |
| `container` | `packages/daemon/src/boot/container.ts`（composition root；service 没在这里 `createXxxService(deps)` 就等于不存在） | `test/integration/container-wiring.test.ts`：`services.*` 全部非空、route 与 job 拿到的是同一实例 |
| `frontendRoutes` | `packages/web/src/app/app.tsx`（路由装配件，`components` 表七键必填）、`packages/web/src/app/routes.tsx`（唯一路由表） | `test/route-entries.test.tsx`：七条 hash 各渲染真实容器 |
| `buildPipeline` | `packages/web/vite.config.ts`、`packages/web/postcss.config.cjs`、`packages/web/tailwind.config.ts` | `test/style-pipeline.test.ts` 与 M1-T11 端到端冒烟：产物 CSS 含 utility、浏览器里 token 生效 |
| `sharedTypes` | `packages/shared/src/api/`（DTO 与请求体类型，一资源一文件） | `test/arch/shared-contracts.test.ts`：无 `unknown[]`、无 `state: string`、枚举只定义一处 |
| `eventKinds` | `packages/shared/src/api/events.ts`（`EVENT_KINDS` 与判别联合） | 事件从 bus 到 SSE 到 DOM 的链路由 M9-T26 与 M1-T11 断言 |
| `migrations` | `packages/daemon/migrations/`（只增不改，序号连续） | `db/migrate.ts` 启动执行；新列必须可空或带默认（09 节） |
| `jobs` | `packages/daemon/src/boot/container.ts` 的 `jobs` 数组（不在数组里的 job 永远不会 `start()`）、`packages/daemon/src/jobs/` | `test/arch/container-jobs.test.ts`：名字集合恰为 08 节的五个 |
| `shellBridge` | `packages/web/src/shell/shell-bridge.ts`（web 侧唯一实现，按平台自装适配器）、`packages/shell-desktop/src-tauri/src/lib.rs`（`generate_handler!` 命令集合） | `packages/shell-desktop/test/tauri-commands.test.ts`：两侧命令名集合相等；M10-T6 的壳 smoke |

**规则**：注册点文件被改动即视为接线变更，审查方逐条核「从入口可达」；任务表验收标准里禁止出现
「占」「位」连写、「桩」、「待接入」、「后续接入」、「后续任务接」与英文大写待办标记（`review.py` X17 阻断）——
接线只能由具名任务在本表登记的注册点上完成，不能留给「以后」。
