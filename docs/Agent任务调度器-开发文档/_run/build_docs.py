...
document.addEventListener("click", function(ev){
  ...
  if(cp && cp.dataset.kind){
    ...
    var row = cp.closest(".htask");
    var txt = promptFor(kind, taskId);
    if(txt) copyText(txt, cp, function(){
      /* 交接台的主操作就是派出实施任务。复制成功后立刻记为进行中，
         否则后续依赖永远看不到进度；卡片里的普通复制不改变派活状态。 */
      if(kind === "impl" && row && stOf(taskId) === "todo"){
        setSt(taskId, "doing");
        refreshHand();
      }
    });
...
function copyText(txt, btn, onCopied){
  function flash(ok){
  ...
  if(navigator.clipboard && navigator.clipboard.writeText){
    navigator.clipboard.writeText(txt).then(function(){ finish(true); }, fallback);
  } else fallback();
}
...
function renderHandoff(){
  ...
  h += '<section class="blk"><h2>按批次派活<span class="k">HANDOFF</span></h2>' +
    ...
    "点「实施」复制成功后自动记为进行中；审查通过并完成落地后，点「进行中」标为「已落地」。" +
    "每次状态变化，上面的窗口调度和下面的可派集合都会重算。</p>" +
...
C.handbatch = function(){
  ...
  return '<div class="hand" id="handbox">' + handBody() + "</div>" +
    ...
    "<span>点「实施」会自动记为进行中；审查通过并落地后点状态标签，后续批次才会解锁</span>" +
    "<span>" + tip + "</span>" +
