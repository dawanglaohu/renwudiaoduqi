  ...
  if(e){
    ...
    h += "<dl><dt>任务</dt><dd>"+esc(t.title||"—")+"</dd>"+
         ...
         '<button class="cp" data-kind="impl" data-task="'+esc(id)+'"'+
           (implLocked(id) ? ' disabled title="依赖还没落地，派了也是白做"' : '')+'>复制实施提示词</button>'+
...
function skillsFor(t, side){
  ...
  if(side === "impl"){
    ...
    if(fe) S.push(["finesse-ui", "写组件与样式时", "组件八态、可访问性基线、手机不塌；register 与 token 已定死，它只帮你落地，不重新设计"]);
    ...
    S.push(["dsh-merging-stacked-prs", "合并时", "核栈成员与顺序、每层 open 且非 draft、等 MERGED、零依赖才删分支。" +
...
function implLocked(id){ var t = taskById(id); return !!(t && waitingOn(t).length); }
...
function designBlock(mod){
  ...
  L.push("");
  L.push("完整说明在 11 节「视觉方向」。查组件写法调 finesse-ui，但不许它重判 register、重定方向——这一趟只把 token 落成代码。");
  return L.join("\n");
}
...
function archBlock(mod){
  ...
  var sh = archLines(a.shared), own = archLines(fe ? a.frontend : a.backend);
...
function roleBlock(t, side){
  ...
  if(side === "impl"){
    L.push("本项目的" + who + "开发工程师。" +
           ...
           "被约定挡住就停下在回报里说明，别绕过去另起一套——下一个任务不知道，第三个又会起第三套。");
  }else{
...
function buildImpl(t){
  ...
  var az = archBlock(t.module); if(az) L.push(az);
  var dz = designBlock(t.module); if(dz) L.push(dz);
  var ps = pathsOf(t.id);
  if(ps.length){
    L.push(""); L.push("## 只改这些路径");
    ps.forEach(function(p){ L.push("- " + p); });
    var rv = rivalsOf(t.id);
    L.push("范围外的文件不动——别人可能正在另一个会话里改，合并时就是冲突；非改不可先停下说明理由。" +
  ...
  L.push("## 步骤");
  if((t.deps||[]).length){
    L.push("1. 切层，它叠在 " + t.deps.join("、") + " 之上：gh stack add " + br +
           "（已建过就 gh stack checkout " + br + "）。别用 init，那会另起一条栈。");
  }else{
    L.push("1. 切层，它没有前置、是本栈最底层：gh stack init " + br + "（已建过就 gh stack checkout " + br + "）");
  }
  L.push("2. 实现，逐条对照验收标准与边界自检。");
  L.push("3. 提交、推送、开 PR（审查方靠 PR 看 diff，没有 PR 它无从下手）：");
  L.push("   git add -A && git commit -m \"" + t.id + " " + shq(t.title) + "\"");
  L.push("   gh stack push && gh stack submit --auto --open");
  L.push("4. 回填知识库（下一节），再交审查。落地由审查方 gh stack merge，你不合，也不碰别层分支。");
  L.push("");
  L.push("## 交活前回填");
  ...
  L.push("");
  L.push("## 不要做");
  L.push("- 不改开发文档，不手改图谱正文（受保护区块除外）；文档有问题写进回报，别自己动手");
  L.push("- 不做别的任务、不提前做后面的；不引入技术栈之外的依赖，确实需要先停下说明");
  L.push("");
  L.push("## 收到返工指令时");
...
function buildReview(t){
  ...
  var ps = pathsOf(t.id);
  if(ps.length){
    L.push(""); L.push("## 允许改动的路径");
    ps.forEach(function(p){ L.push("- " + p); });
    L.push("范围外的改动即使改得对也记进 OUT_OF_SCOPE——并行开发时那是冲突来源。");
  }
  ...
  L.push("   没有 PR 就自己补开，不算实施方的错：gh stack checkout " + br + " && gh stack submit --auto --open");
  ...
  L.push("");
  L.push("## 分级：只有阻断项才打回");
  L.push("阻断项：验收标准不满足；边界没处理；范围外改动且影响别的任务；违反 06/07/08 的分层与共用约定；数据会丢或错、权限能绕；测试红。");
  L.push("非阻断项（命名、注释、小重复、格式、回填措辞、几行改完且不改行为的小毛病）不构成 rework：" +
         "不超过约 30 行、不动接口与数据结构的你自己顺手改——gh stack checkout " + br +
  ...
  L.push("  第 2 轮只核上一轮条目，不新增无关阻断项。**最多两轮**：之后剩的具体代码错误由你改掉、加测试、提交、push，" +
         "记进 FIXED_BY_REVIEWER，按 pass 落地；剩的是方向性问题（方案不对、任务定义不可实现）→ doc-issue 或停下报给用户。");
  L.push("- doc-issue：问题在文档（约定含糊、章节打架、边界写了等于没写、任务不可实现）→ 停手改文档（拿不准放哪节调 dsh-doc-standards），" +
         "重跑 review.py、build_docs.py、build_vault.py（产物不能手改，见 dsh-doc-site-sync），用刷新后的实施提示词重派。");
  L.push("");
  L.push("## 落地（pass 之后一口气做完）");
  L.push("1. 回填：" + note + " 的 code / notes 两个受保护区块。实施方填了就核质量（dsh-prose-standard、dsh-trim-cot-leakage 各过一遍），" +
         "缺了或写成废话就自己补。跑 python " + HDOCS + "/_run/build_vault.py " + HDOCS);
  L.push("2. 记录落地：python " + HDOCS + "/_run/build_docs.py " + HDOCS + " --landed " + t.id +
         "　（写进任务笔记的 status 与 _run/progress.js；交接台刷新后这一行变「已落地」，下游解锁）");
  L.push("3. 一起进仓库：gh stack checkout " + br + " && git add " + HDOCS + " && git commit -m \"" + t.id + " 回填知识库并记录落地\" && gh stack push");
  L.push("4. 证据：调 dsh-pre-push-checks 按本层改动挑最小充分测试集跑一遍；红了回头按阻断项处理");
  L.push("5. 还是 draft 就 gh pr ready " + br);
  L.push("6. gh stack merge <本层 PR 号> --yes --merge　" + (up.length
      ? "本层连同下面未合的层一起合；依赖本层的 " + up.map(function(x){ return x.id; }).join("、") +
        " 正常还没开工，若已叠在上面，其 PR base 由 GitHub 改到主干，让它们的会话跑 gh stack sync"
      : "本层是栈顶，合它就是整栈落地") + "。等每个 PR 都报 MERGED 才算落地");
  ...
  L.push("REWORK");
  L.push("- R1 现象 → 文件:行 → 改成什么样 → 怎么验证；pass 写 none");
  ...
  L.push("- 一行给用户，三选一：「把下面的返工指令发给实施模型，它推送后再点「审查」」｜" +
         "「已 merge 并记录落地，刷新交接台派下一批（没变就手点状态标签）」｜「文档要改：<哪节哪条>，重跑三个脚本再重派」");
  L.push("");
...
function buildBug(t){
  ...
  L.push("- 代码在哪：" + (landed
      ...
      : "栈分支 " + br + "，还没合：gh stack checkout " + br + "；gh pr diff " + br + " 看本层改了什么"));
  L.push("");
  ...
  L.push(landed
      ? "- 代码已在主干，修复另开一条栈，不直接推主干：gh stack init fix/" + t.id + "-<一两个词> → git commit -m \"fix(" + t.id +
  ...
  L.push("NOT_FIXED");
  L.push("- B2 → 为什么不修（根因在范围外 / 要改文档 / 要用户决定）→ 建议");
  L.push("SUSPECT");
...
function buildBugAll(){
  ...
  L.push("- 修复统一开一条栈：gh stack init fix/<日期>-<一两个词>；相关的几处修在同一层，不相关的各起一层（gh stack add）。" +
  ...
  L.push("- B1 → commit <hash>：改了什么、加了哪个测试");
  L.push("NOT_FIXED");
  L.push("- B2 → 为什么不修（要改文档 / 要用户决定 / 牵动太大该单开任务）→ 建议");
  L.push("SUSPECT");
  L.push("- 怀疑但没复现的；没有写 none");
  L.push("NEXT");
...
function promptFor(kind, id){
  ...
  if(kind === "review") return buildReview(t);
...
function handBody(){
  ...
  var h = '<div class="kick">' +
    ...
    "（自动推到审查中）；审查判 pass 落地时会把「已落地」记进任务笔记，刷新本页就解锁下游，没记录的手点状态标签。" +
...
def write_progress(root, st):
    ...
    if not os.path.isdir(rd):
        os.makedirs(rd)
    with open(os.path.join(rd, "progress.js"), "w", encoding="utf-8") as f:
        f.write("// 任务落地记录。由 build_docs.py 从 图谱/任务/<ID>.md 头部的 status 派生，或由 --landed 写入；\n"
                "// 阅读器加载时与本机点出来的进度合并，取更靠后的一格。撤销：把笔记的 status 改回 todo 再重跑。\n"
...
def mark_landed(root, ids):
    ...
    known = {t["id"] for t in extract(by_num)["tasks"]} if groups else set()
    bad = [i for i in ids if known and i not in known]
    ok = []
    for i in ids:
        if i in bad:
            continue
